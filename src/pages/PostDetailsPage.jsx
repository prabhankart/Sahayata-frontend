...
const PostDetailsPage = () => {
  ...
  const handleStatusChange = async (status) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await axios.put(`${API_URL}/api/posts/${post?._id}/status`, { status }, config);
      setPost(data);
  ...
  const isPledgedByCurrentUser = post?.pledgedBy?.some(p => (p._id || p) === user?._id);
  const isPostCreator = user?._id && post?.user?._id && user._id === post.user._id;
  const canViewChat = isPostCreator || isPledgedByCurrentUser;

  return (
    <>
      {reviewModalOpen && (
        <ReviewModal post={post} reviewee={reviewee} onClose={() => setReviewModalOpen(false)} />
      )}
      ...
      <div className="relative bg-gradient-to-b from-gray-50 via-white to-violet-50/50 min-h-[calc(100vh-4rem)] py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
          ...
          <div className="lg:col-span-2 bg-white/90 backdrop-blur rounded-3xl border border-zinc-200/70 shadow-[0_12px_40px_-12px_rgba(24,24,27,0.25)]">
            {post?.image && (
              <div className="overflow-hidden rounded-t-3xl">
                <MediaDisplay url={post.image} alt={post?.title || 'Post image'} />
              </div>
            )}
              ...
              <div className="flex justify-between items-start gap-4">
                ...
                {isPostCreator && (
                  <div className="flex space-x-2 flex-shrink-0 ml-4">
                    <Link to={`/post/${post._id}/edit`} className="p-2 rounded-lg hover:bg-gray-100 ring-1 ring-transparent hover:ring-gray-200 transition">
                      <PencilIcon className="h-5 w-5 text-gray-600" />
                    </Link>
                    <button onClick={handleDelete} className="p-2 rounded-lg hover:bg-rose-50 ring-1 ring-transparent hover:ring-rose-200 transition">
          ...
          <div className="lg:col-span-1 space-y-8 lg:sticky lg:top-24">
            <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-[0_10px_30px_-12px_rgba(24,24,27,0.25)] border border-zinc-200/70">
              <h3 className="text-xl font-bold text-gray-900 border-b border-zinc-200/70 pb-3 mb-4">Details</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Posted by:</span>
                  {post?.user ? (
                    <Link to={`/profile/${post.user._id}`} className="font-semibold text-gray-900 hover:text-primary">
                      {post.user.name}
                    </Link>
                  ) : (
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-yellow-700">{post?.status || 'Open'}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{post?.urgency || '—'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-[0_10px_30px_-12px_rgba(24,24,27,0.25)] border border-zinc-200/70">
              ...
              {post?.pledgedBy?.length > 0 ? (
                <ul className="space-y-4">
                  {post.pledgedBy.map(helper => (
                    <li key={helper._id || helper} className="flex justify-between items-center">
                      <Link to={`/profile/${helper._id || helper}`} className="text-emerald-700 font-semibold hover:text-emerald-800">
                        {helper.name || 'Unknown'}
                      </Link>
                      {isPostCreator && post?.status === 'Resolved' && (
                        <button onClick={() => openReviewModal(helper)} className="text-xs text-primary font-semibold hover:underline">
                          Leave Review
                        </button>
                      )}
                    </li>
              ...
              {isPledgedByCurrentUser && post?.status === 'Resolved' && (
                <div className="border-t border-gray-200 mt-4 pt-4 flex justify-between items-center">
                  ...
                  <button onClick={() => openReviewModal(post?.user)} className="text-xs text-primary font-semibold hover:underline">
                    Leave Review
                  </button>
                </div>
              )}
            ...
            <div className="min-h-[450px]">
              {user && canViewChat && (
                <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-[0_10px_30px_-12px_rgba(24,24,27,0.25)] border border-zinc-200/70 h-full flex flex-col">
                  <h3 className="text-xl font-bold text-gray-900 border-b border-zinc-200/70 pb-3 mb-4 flex-shrink-0">Project Chat</h3>
