        ...
        <div className="flex-grow ml-4">
          {currentPost.image && (
            <Link to={`/post/${currentPost._id}`}>
              <img src={currentPost.image} alt={currentPost.title} className="w-full h-40 object-cover rounded-md mb-4 hover:opacity-90" />
            </Link>
          )}
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
          ...
          {user && user._id !== currentPost.user._id && (
            <button
              ...
              className={`font-semibold py-2 px-4 rounded-full text-xs shadow-lg transform hover:scale-105 transition-all duration-300 ${
